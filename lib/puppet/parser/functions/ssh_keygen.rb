...
#     :dir    (the subdir of /etc/puppet/ to store the key in - default: 'ssh', or a full path to a directory)
#     :size   (the key size - default 4096)
  ...
  newfunction(:ssh_keygen, :type => :rvalue) do |args|
    ...
    begin
      unless File.exists?("#{fullpath}/#{config['name']}") then
        %x[/usr/bin/ssh-keygen -t #{config['type']} -b #{config['size']} -P '' -f #{fullpath}/#{config['name']} -C "#{config['name']} created by Puppet on $(date)"]
      end
    rescue => e
